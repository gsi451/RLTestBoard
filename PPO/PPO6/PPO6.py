...
def preprocess_df(df):
    ...
    for col in ["open", "high", "low", "close", "Bollinger_Upper", "Bollinger_Lower"]:
...
def finalize_df(df):
    df_final = df[["adj_open", "adj_high", "adj_low", "adj_close", "adj_Bollinger_Upper", "adj_Bollinger_Lower", "log_scaled_volume"]].copy()
    df_final.columns = ["open", "high", "low", "close", "volume", "Bollinger_Upper", "Bollinger_Lower"]
    return df_final
...
# 강화학습 환경에서 사용될 피처 컬럼
FEATURE_COLUMNS = ['open', 'high', 'low', 'close', "Bollinger_Upper", "Bollinger_Lower", 'volume']
...
# 학습된 모델 저장
model.save("ppo_trading_model_ppo6")
print("모델 저장 완료!")
